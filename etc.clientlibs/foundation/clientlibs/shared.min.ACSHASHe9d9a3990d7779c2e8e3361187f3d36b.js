window._g = window._g || {};
_g.shared = {};
if (window.console === undefined) window.console = { log: function (m) {} };
_g.shared.HTTP = new (function () {
  ...
  return {
    ...
    post: function (
      ...
      if (callback != undefined)
        return _g.$.ajax({
          ...
          hook: false,
          complete: function (request, textStatus) {
            var response = _g.HTTP.buildPostResponseFromHTML(
    ...
    getParameters: function (url, name) {
      ...
      var queryPts = query.split("\x26");
      for (var i = 0; i < queryPts.length; i++) {
        var paramPts = queryPts[i].split("\x3d");
        if (paramPts[0] == name)
          values.push(
            paramPts.length > 1 ? decodeURIComponent(paramPts[1]) : ""
          );
      }
    ...
    addParameter: function (url, name, value) {
      ...
      var separator = url.indexOf("?") == -1 ? "?" : "\x26";
    ...
    removeParameter: function (url, name) {
      var pattern0 = "?" + encodeURIComponent(name) + "\x3d";
      var pattern1 = "\x26" + encodeURIComponent(name) + "\x3d";
      var pattern;
      ...
      var indexCutEnd = url.indexOf("\x26", indexCutStart + 1);
      var end = "";
      if (indexCutEnd != -1) {
        end = url.substring(indexCutEnd);
        if (end.indexOf("\x26") == 0) end = end.replace("\x26", "?");
      }
      return begin + end;
    },
    ...
    setSelector: function (url, selector, index) {
      ...
      var fragment =
        selectors.length > 0 ? url.replace("." + selectors.join("."), "") : url;
      if (selectors.length > 0)
        for (var i = 0; i < selectors.length; i++)
          if (index == i) fragment += "." + selector;
    ...
    buildPostResponseFromNode: function (node, response) {
      ...
      for (var i = 0; i < node.childNodes.length; i++) {
        ...
        if (child.tagName) {
          if (child.id)
            if (child.href) response.headers[child.id] = child.href;
    ...
    buildPostResponseFromHTML: function (html) {
      var response = createResponse();
      try {
        if (html.responseText != undefined) html = html.responseText;
    ...
    getCookie: function (name) {
      var cname = encodeURIComponent(name) + "\x3d";
    ...
    setCookie: function (name, value, path, days, domain, secure) {
      ...
      document.cookie =
        ...
        encodeURIComponent(value) +
        "; " +
        (days != 0 ? "expires\x3d" + date.toGMTString() + "; " : "") +
        (domain ? "domain\x3d" + domain + "; " : "") +
        (path ? "path\x3d" + path : "") +
        (secure ? "; secure" : "");
      return value;
    },
    ...
    externalize: function (url, encode) {
      if (typeof G_IS_HOOKED != "undefined" && G_IS_HOOKED(url)) return url;
    ...
    getSuffix: function () {
      if (window.CQURLInfo && CQURLInfo.suffix) return CQURLInfo.suffix;
      return null;
    },
    getSelectors: function (url) {
      if (!url && window.CQURLInfo)
        if (CQURLInfo.selectors) return CQURLInfo.selectors;
      var selectors = [];
      ...
      if (fragment) {
        ...
        if (split.length > 2)
          for (var i = 0; i < split.length; i++)
            if (i > 0 && i < split.length - 1) selectors.push(split[i]);
      }
      return selectors;
    },
    getExtension: function (url) {
      if (!url && window.CQURLInfo)
        if (CQURLInfo.extension) return CQURLInfo.extension;
      url = url || window.location.href;
      url = _g.HTTP.removeParameters(url);
      url = _g.HTTP.removeAnchor(url);
    ...
    reloadHook: function (url) {
      if (
        typeof G_RELOAD_HOOK != "undefined" &&
...
_g.shared.Sling = (function () {
  return {
    ...
    getContentPath: function (relPath, absPath, allowParentPaths) {
      ...
      if (relPath)
        if (relPath.indexOf("/") == 0) path = relPath;
        else {
          if (allowParentPaths)
...
_g.shared.XSS = new (function () {
  return {
    getXSSPropertyName: function (propertyName) {
      ...
      if (_g.XSS.KEY_REGEXP.test(propertyName)) return propertyName;
    ...
    getXSSRecordPropertyValue: function (rec, propertyName, ellipsisLimit) {
      ...
      if (rec && propertyName) {
        ...
        if (xssPropValue) value = xssPropValue;
        else value = this.getXSSValue(rec.get(propertyName));
    ...
    getXSSTablePropertyValue: function (table, propertyName, ellipsisLimit) {
      ...
      if (table && propertyName) {
        ...
        if (xssPropValue) value = xssPropValue;
        else value = this.getXSSValue(table[propertyName]);
        if (ellipsisLimit && !isNaN(ellipsisLimit))
          value = _g.Util.ellipsis(value, ellipsisLimit, true);
      }
      return value;
    ...
    getXSSValue: function (val) {
      if (val) return _g.Util.htmlEncode(val);
      else return "";
    },
    updatePropertyName: function (cfg, propertyName) {
      if (!cfg || !propertyName || !cfg[propertyName]) return;
    ...
    xssPropertyRenderer: function (val, meta, rec, cfg) {
      if (
        ...
        rec &&
        rec.data &&
        rec.data[this.getXSSPropertyName(cfg["dataIndex"])]
      ) {
...
_g.shared.ClientSidePersistence = function (cfg) {
  var session = {
    ...
    debug: function () {
      if (console) {
        ...
        for (
          var idx = 0, keys = Object.keys(map).sort(), last = null;
          idx < keys.length;
          idx++
        ) {
          var key = keys[idx];
          ...
          debugInfo +=
            ...
            ++count +
            "]-\x3e '" +
            key.replace(containerRE, "") +
            "' \x3d '" +
            decodeURIComponent(value) +
            "'\n";
        }
        if (!count) debugInfo += "(container is empty)";
    ...
    set: function (key, value) {
      ...
      if (value) map[key] = encodeURIComponent(value);
      else {
        eventData.action = "remove";
        delete map[key];
      }
      for (var entry in map) result.push(entry + ":\x3d" + map[entry]);
      this.cache = map;
    ...
    getMap: function () {
      if (!this.cache || !this.config.useCache) {
        ...
        for (var idx = 0; idx < data.length; idx++) {
          var chunks = data[idx].split(":\x3d");
          var key = chunks[0];
          if (key && key.length) result[key] = chunks[1] || "";
        }
        this.cache = result;
      }
      return this.cache;
  ...
  _g.$.extend(
    session.config,
    _g.shared.ClientSidePersistence.getDefaultConfig(),
    cfg
  );
  if (session.config.useContainer === false) session.config.container = null;
  var useFallback;
